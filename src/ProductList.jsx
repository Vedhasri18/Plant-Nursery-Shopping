...
function ProductList() {
  ...
  const handlePlantsClick = (e) => {
    e.preventDefault();
    setShowPlants((prevState) => !prevState); // Set showAboutUs to true when "About Us" link is clicked
    setShowCart(false); // Hide the cart when navigating to About Us
  };

  const handleContinueShopping = (e) => {
    if (e && e.preventDefault) {
        e.preventDefault(); // Prevent default only if e exists
    }
    setShowCart(false);
  };
  const handleAddToCart = (product) => {
    ...
    setAddedToCart((prevState) => ({
      ...prevState,
      [product.name]: true, // Use product ID as key
    }));
  };
